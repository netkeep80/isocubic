...
**Статус**: 🔄 В процессе (TASK 82 завершён)
...
| **Phase 2: Валидация и тесты** | Неделя 4 | Схема + расширенные правила + генерация тестов | 📋 Запланирована |
| **Phase 3: AI-интеграция** | Неделя 5 | Промпты, контекст-билдер, pre-commit hook | 📋 Запланирована |
...
├── metamode-semantic-validator.ts   # Phase 0: Семантическая валидация
...
**Описание**:
Расширить систему валидации: добавить JSON Schema для `@mm:` аннотаций, усилить семантические правила, реализовать AI-генерацию тестов на валидность метаданных.

**Задачи**:

- [ ] Создать `schemas/mm-annotation.schema.json` для полной валидации `@mm:` формата
- [ ] Добавить правило `schema-validates` в семантический валидатор
- [ ] Реализовать `metamode-test-generator.ts` для AI-генерации тестов
- [ ] Добавить `metamode:generate-tests` в npm скрипты
- [ ] Обновить Vite плагин для запуска валидации при сборке
- [ ] Написать тесты для схемного валидатора

**Критерии приёмки**:

- [ ] Сборка падает при нарушении аннотационной схемы
- [ ] AI-генератор создаёт рабочие тесты для любого модуля с `@mm:id`
- [ ] Все тесты проходят
...
**Описание**:
Создать специализированный контекст-билдер для AI-агентов (CodeGen, Refactor, DocGen) на основе v2.0 БД. Разработать промпт-шаблоны и pre-commit hook.

**Задачи**:

- [ ] Создать `scripts/metamode-context-builder.ts` для формирования AI-контекста
- [ ] Разработать промпт-шаблоны для разных типов AI-агентов
- [ ] Реализовать pre-commit hook для добавления `@mm:` при создании файлов
- [ ] Добавить команды `metamode:context` и `metamode:ai:context`
- [ ] Написать тесты для контекст-билдера
...
- [ ] Убедиться во всех CLI командах по спецификации
- [ ] Запустить и верифицировать полную миграцию isocubic
- [ ] Добавить dual-mode (v1 и v2 работают параллельно)
...
**Описание**:
Создать полную документацию для MetaMode v2.0: гайды, примеры, migration guide, обновление README.

**Задачи**:

- [ ] Обновить `metamode.md` с полной спецификацией v2.0
- [ ] Создать `docs/metamode-v2-migration.md`
- [ ] Добавить примеры в `examples/`
- [ ] Финально обновить `README.md`
...
| 83. Валидация и тесты (Phase 2) | Средняя | Высокий | 📋 Запланирована |
| 84. AI-интеграция (Phase 3) | Средняя | Высокий | 📋 Запланирована |

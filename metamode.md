...
MetaMode — это унифицированная система метаинформации проекта, объединяющая функциональность MetaNet, DevMode, GodMode и VueDevMode в единую компактную систему, оптимизированную для работы с AI-агентами.
...
1. Inline `@metamode` в компоненте (высший приоритет)
...
Файл `metamode.schema.json` содержит JSON Schema (draft-07) для валидации:

- Проверка обязательных полей (`name`, `desc`)
- Проверка типов и enum-значений
- Ограничение допустимых полей (`additionalProperties: false`)
- Валидация семантической версии

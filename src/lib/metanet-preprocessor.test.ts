...
describe('MetaNet Preprocessor', () => {
  ...
  describe('Schema validation', () => {
    it('should validate a minimal valid metanet.json', () => {
  ...
  describe('File reference checks', () => {
    it('should pass when referenced files exist', () => {
      const schemaPath = path.resolve('metanet.schema.json')
      fs.copyFileSync(schemaPath, path.join(tmpDir, 'metanet.schema.json'))
    ...
    it('should error when referenced file does not exist', () => {
      const schemaPath = path.resolve('metanet.schema.json')
      fs.copyFileSync(schemaPath, path.join(tmpDir, 'metanet.schema.json'))
  ...
  describe('Directory reference checks', () => {
    it('should recursively validate subdirectory metanet.json', () => {
      const schemaPath = path.resolve('metanet.schema.json')
      fs.copyFileSync(schemaPath, path.join(tmpDir, 'metanet.schema.json'))
    ...
    it('should error when referenced subdirectory metanet.json does not exist', () => {
      const schemaPath = path.resolve('metanet.schema.json')
      fs.copyFileSync(schemaPath, path.join(tmpDir, 'metanet.schema.json'))
  ...
  describe('Sync warnings', () => {
    it('should warn about files not described in metanet.json when verbose', () => {
      const schemaPath = path.resolve('metanet.schema.json')
      fs.copyFileSync(schemaPath, path.join(tmpDir, 'metanet.schema.json'))
...
describe('MetaNet on actual project', () => {
  it('should validate the actual project metanet.json files without errors', () => {
    const result = execSync(`npx tsx ${path.resolve('scripts/metanet-preprocessor.ts')} --check`, {
      cwd: path.resolve('.'),
      encoding: 'utf-8',
    })
    expect(result).toContain('All metanet.json files are valid')

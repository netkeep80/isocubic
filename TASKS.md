...
# План задач: Фаза 5 — Публикация и шаринг кубиков

Данный раздел содержит последовательность ISSUE для реализации системы публикации и шаринга кубиков.

---

## Обзор Фазы 5

**Цель**: Добавить возможность публикации кубиков в общедоступную галерею, обмена через ссылки и социальные функции.

**Основные требования**:
- Публикация кубиков в облачную галерею
- Генерация share-ссылок для кубиков и коллекций
- Система пользовательских профилей
- Социальные функции (лайки, избранное, комментарии)
- Поиск и фильтрация в публичной галерее
...
### ISSUE 25: Система аутентификации пользователей

**Заголовок**: `Реализация системы аутентификации для публикации кубиков`

**Описание**:
Создать систему аутентификации пользователей для возможности публикации и управления своими кубиками.

**Задачи**:
- [ ] Создать типы для пользователей `/src/types/user.ts`:
  - `User` — профиль пользователя
  - `UserProfile` — публичный профиль
  - `AuthState` — состояние аутентификации
  - `AuthProvider` — поддерживаемые провайдеры (GitHub, Google, email)
- [ ] Создать модуль `/src/lib/auth-client.ts`:
  - OAuth-авторизация через GitHub/Google
  - Email/пароль аутентификация
  - Управление токенами (access/refresh)
  - Выход из системы
- [ ] Создать компонент `/src/components/AuthPanel.tsx`:
  - Форма входа/регистрации
  - Кнопки OAuth-провайдеров
  - Отображение текущего пользователя
  - Меню профиля
- [ ] Интегрировать с серверной частью:
...
**Критерии приёмки**:
- Пользователь может войти через GitHub/Google/email
- Сессия сохраняется при обновлении страницы
- Выход очищает все данные сессии
- Тесты проходят с покрытием >= 80%

**Метки**: `auth`, `user`, `security`

---

### ISSUE 26: Публикация кубиков в облачную галерею

**Заголовок**: `Реализация публикации кубиков в облачное хранилище`

**Описание**:
Создать систему публикации кубиков с сохранением в облачное хранилище и отображением в публичной галерее.

**Задачи**:
- [ ] Создать типы для публикаций `/src/types/publication.ts`:
  - `PublishedCube` — опубликованный кубик
  - `CubeCollection` — коллекция кубиков
  - `PublicationStatus` — статус публикации (draft, pending, published, rejected)
  - `CubeVisibility` — видимость (public, unlisted, private)
- [ ] Создать модуль `/src/lib/publication.ts`:
  - Публикация кубика с метаданными
  - Обновление опубликованного кубика
  - Удаление публикации
  - Получение списка своих публикаций
- [ ] Создать компонент `/src/components/PublishPanel.tsx`:
  - Форма публикации с названием, описанием, тегами
  - Выбор видимости
  - Предпросмотр перед публикацией
  - Индикатор загрузки
- [ ] Создать серверный API:
  - `POST /api/cubes` — публикация
  - `PUT /api/cubes/:id` — обновление
  - `DELETE /api/cubes/:id` — удаление
  - `GET /api/cubes/my` — список своих кубиков
- [ ] Добавить валидацию и санитизацию данных
- [ ] Написать интеграционные тесты

**Критерии приёмки**:
- Кубик успешно публикуется в облако
- Метаданные (теги, описание) сохраняются
- Публикация отображается в профиле пользователя
- Можно удалить или обновить свою публикацию

**Метки**: `publication`, `cloud`, `storage`

---

### ISSUE 27: Публичная галерея кубиков

**Заголовок**: `Создание публичной галереи с поиском и фильтрацией`

**Описание**:
Разработать публичную галерею для просмотра опубликованных кубиков с поиском, фильтрацией и сортировкой.

**Задачи**:
- [ ] Создать компонент `/src/components/PublicGallery.tsx`:
  - Сетка карточек с превью кубиков
...
- [ ] Реализовать фильтрацию:
  - По категориям (природа, архитектура, магия, и т.д.)
  - По материалам (stone, wood, crystal, и т.д.)
  - По автору
  - По дате публикации
- [ ] Реализовать сортировку:
  ...
  - По алфавиту
- [ ] Создать серверный API:
  - `GET /api/cubes` — список с фильтрами и пагинацией
  - `GET /api/cubes/:id` — детали кубика
  - `GET /api/cubes/search` — поиск
- [ ] Оптимизировать производительность запросов
- [ ] Написать тесты для компонентов

**Критерии приёмки**:
- Галерея загружается за < 2 секунды
- Поиск работает мгновенно
- Фильтры комбинируются корректно
- Пагинация работает стабильно

**Метки**: `gallery`, `search`, `ui`

---

### ISSUE 28: Система share-ссылок

**Заголовок**: `Реализация генерации и обработки share-ссылок`

**Описание**:
Создать систему генерации коротких ссылок для быстрого обмена кубиками и коллекциями.

**Задачи**:
- [ ] Создать типы для шаринга `/src/types/share.ts`:
  - `ShareLink` — структура ссылки
  - `ShareOptions` — настройки шаринга
  - `EmbedConfig` — конфигурация встраивания
- [ ] Создать модуль `/src/lib/share.ts`:
  - Генерация короткого кода для кубика
  - Генерация ссылки на коллекцию
  - Разрешение кода в полный URL
  - Экспорт в формате embed-кода
- [ ] Создать компонент `/src/components/SharePanel.tsx`:
  - Кнопка "Поделиться"
  - Копирование ссылки в буфер
  - QR-код для ссылки
  - Embed-код для встраивания
  - Шаринг в соцсети (Twitter, Telegram, и т.д.)
- [ ] Создать страницу просмотра по ссылке:
  - `/cube/:shortcode` — просмотр одного кубика
  - `/collection/:shortcode` — просмотр коллекции
  - OG-теги для превью в соцсетях
- [ ] Создать серверный API:
  - `POST /api/share` — создание share-ссылки
  - `GET /api/share/:code` — получение данных по коду
- [ ] Добавить аналитику переходов по ссылкам
- [ ] Написать тесты

**Критерии приёмки**:
- Ссылки генерируются за < 100ms
- Ссылки работают без аутентификации
- Превью корректно отображается в соцсетях
- QR-код сканируется без проблем

**Метки**: `share`, `links`, `social`

---

### ISSUE 29: Социальные функции (лайки, избранное)

**Заголовок**: `Реализация системы лайков и избранного`

**Описание**:
Добавить социальные функции для взаимодействия с опубликованными кубиками.

**Задачи**:
- [ ] Создать типы для социальных функций `/src/types/social.ts`:
  - `Like` — структура лайка
  - `Favorite` — избранное
  - `SocialStats` — статистика (лайки, просмотры, избранное)
- [ ] Создать модуль `/src/lib/social.ts`:
  - Добавление/удаление лайка
  - Добавление/удаление из избранного
  - Получение статистики кубика
  - Получение списка избранного пользователя
- [ ] Обновить компонент карточки кубика:
  - Кнопка лайка с счётчиком
  - Кнопка добавления в избранное
  - Отображение количества просмотров
- [ ] Создать страницу избранного `/favorites`:
  - Список избранных кубиков пользователя
  - Организация по папкам/категориям
  - Быстрый доступ к редактированию
- [ ] Создать серверный API:
  - `POST /api/cubes/:id/like` — лайк
  - `DELETE /api/cubes/:id/like` — убрать лайк
  - `POST /api/cubes/:id/favorite` — в избранное
  - `DELETE /api/cubes/:id/favorite` — из избранного
  - `GET /api/user/favorites` — список избранного
- [ ] Добавить real-time обновление счётчиков
- [ ] Написать тесты
...
- Действия требуют аутентификации

**Метки**: `social`, `likes`, `favorites`

---

### ISSUE 30: Профили пользователей

**Заголовок**: `Создание публичных профилей пользователей`

**Описание**:
Разработать систему публичных профилей с галереей опубликованных кубиков и статистикой.

**Задачи**:
- [ ] Создать компонент `/src/components/UserProfile.tsx`:
  - Аватар и имя пользователя
  - Биография
  - Ссылки на соцсети
  - Галерея опубликованных кубиков
  - Статистика (публикации, лайки, подписчики)
...
- [ ] Создать серверный API:
  ...
  - `DELETE /api/users/:id/follow` — отписка
  - `GET /api/users/me/feed` — лента подписок
- [ ] Написать тесты

**Критерии приёмки**:
- Профиль загружается за < 1 секунду
- Аватар отображается корректно
- Подписки работают надёжно
- Лента обновляется в реальном времени

**Метки**: `profile`, `user`, `social`

---

### ISSUE 31: Система комментариев

**Заголовок**: `Реализация системы комментариев к кубикам`

**Описание**:
Добавить возможность оставлять комментарии к опубликованным кубикам.

**Задачи**:
- [ ] Создать типы для комментариев `/src/types/comments.ts`:
  - `Comment` — структура комментария
  - `CommentThread` — ветка комментариев
  - `CommentReaction` — реакции на комментарии
- [ ] Создать модуль `/src/lib/comments.ts`:
  - Добавление комментария
  - Редактирование своего комментария
  - Удаление комментария
  - Ответ на комментарий (вложенные треды)
  - Реакции на комментарии
- [ ] Создать компонент `/src/components/CommentSection.tsx`:
...
- [ ] Создать серверный API:
  - `GET /api/cubes/:id/comments` — список комментариев
  - `POST /api/cubes/:id/comments` — добавить комментарий
...
**Критерии приёмки**:
- Комментарии загружаются быстро
- Вложенность работает корректно
- Markdown рендерится безопасно
- Модерация предотвращает спам

**Метки**: `comments`, `social`, `moderation`

---

### ISSUE 32: Система модерации контента

**Заголовок**: `Реализация системы модерации публикаций`

**Описание**:
Создать систему модерации для обеспечения качества публичного контента.

**Задачи**:
- [ ] Создать типы для модерации `/src/types/moderation.ts`:
  - `ModerationReport` — жалоба на контент
  - `ModerationAction` — действие модератора
  - `ModerationStatus` — статус проверки
  - `ContentPolicy` — правила контента
- [ ] Создать модуль `/src/lib/moderation.ts`:
  - Отправка жалобы на кубик/комментарий
  - Автоматическая проверка (ML-фильтры)
  - Очередь модерации
  - Применение санкций
- [ ] Создать панель модератора:
  - Очередь жалоб
  - Просмотр контента
  - Принятие решений (одобрить/отклонить/удалить)
  - История модерации
- [ ] Реализовать автомодерацию:
  - Фильтрация запрещённого контента
  - Проверка на дубликаты
  - Проверка на спам
- [ ] Создать серверный API:
  - `POST /api/report` — отправить жалобу
  - `GET /api/moderation/queue` — очередь (для модераторов)
  - `POST /api/moderation/:id/action` — действие модератора
- [ ] Написать тесты

**Критерии приёмки**:
- Жалобы обрабатываются в течение 24 часов
- Автомодерация блокирует очевидный спам
- История модерации сохраняется
- Пользователи получают уведомления о решениях

**Метки**: `moderation`, `safety`, `admin`
...
```mermaid
graph TD
    I24[ISSUE 24: Серверная часть] --> I25[ISSUE 25: Аутентификация]
    I25 --> I26[ISSUE 26: Публикация]
    I26 --> I27[ISSUE 27: Публичная галерея]
    I26 --> I28[ISSUE 28: Share-ссылки]
    I25 --> I29[ISSUE 29: Лайки и избранное]
    I27 --> I29
    I25 --> I30[ISSUE 30: Профили]
    I29 --> I30
    I26 --> I31[ISSUE 31: Комментарии]
    I29 --> I31
    I26 --> I32[ISSUE 32: Модерация]
    I31 --> I32
```
...
| ISSUE | Сложность | Приоритет |
|-------|-----------|-----------|
| 25. Аутентификация | Высокая | Критический |
| 26. Публикация | Средняя | Критический |
| 27. Публичная галерея | Средняя | Критический |
| 28. Share-ссылки | Низкая | Высокий |
| 29. Лайки и избранное | Низкая | Высокий |
| 30. Профили пользователей | Средняя | Средний |
| 31. Комментарии | Средняя | Средний |
| 32. Модерация | Высокая | Средний |
...
- **Фаза 6**: Marketplace и монетизация

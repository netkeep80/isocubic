...
- [x] Добавить fallback на polling для устаревших браузеров
...
# План задач: Фаза 5 — Расширение редактора параметров

Данный раздел содержит последовательность ISSUE для расширения ParamEditor для поддержки всех нововведений из предыдущих фаз.
...
**Описание**:
Добавить в ParamEditor секцию для редактирования параметров сшивки границ кубиков (CubeBoundary), которые были реализованы в ISSUE 7, но не были добавлены в редактор.

**Задачи**:
- [ ] Расширить `ParamEditor.tsx` новой секцией "Boundary Settings":
  - Dropdown для `boundary.mode`: none / smooth / hard
  - Слайдер для `boundary.neighbor_influence` (0.0 - 1.0)
  - Описания для каждого режима
- [ ] Добавить визуальную подсказку о влиянии настроек на сетку кубов
- [ ] Создать callback `updateBoundary` аналогично `updateBase`, `updateNoise`
- [ ] Добавить тесты для новой секции

**Критерии приёмки**:
- Пользователь может изменять режим границ через UI
- Изменения применяются в реальном времени к CubeGrid
...
### ISSUE 26: Базовый редактор FFT-кубиков (FFT Cube Editor)

**Заголовок**: `Создание редактора для магических/энергетических кубиков (FFTCubeConfig)`

**Описание**:
Создать новый режим редактора или расширение ParamEditor для работы с FFT-кубиками (магическими объектами), реализованными в Фазе 2.

**Задачи**:
- [ ] Создать компонент `/src/components/FFTParamEditor.tsx` или расширить ParamEditor:
  - Переключатель режима: SpectralCube / FFTCubeConfig
  - Секция "Energy Settings":
    - Checkbox `is_magical`
    - Dropdown для `fft_size`: 8 / 16 / 32
    - Числовое поле `energy_capacity`
    - Индикатор `current_energy` (только чтение)
- [ ] Секция "FFT Physics":
  - Слайдер `coherence_loss` (0.0 - 0.1)
  - Слайдер `fracture_threshold` (0 - 200)
  - Визуальный индикатор stress level
- [ ] Интеграция с энергетическими функциями из `energyPhysics.ts`:
  - Отображение getNormalizedEnergy()
  - Предупреждение при isNearFracture()
- [ ] Добавить типы и интерфейсы для FFT-редактора
- [ ] Написать тесты

**Критерии приёмки**:
- Пользователь может переключаться между обычным и FFT-режимом
- Все FFT-параметры редактируемы
- Индикаторы энергии обновляются в реальном времени
- Предупреждение при приближении к fracture threshold
...
**Описание**:
Добавить возможность редактирования FFT-коэффициентов для каждого канала (R, G, B, A) с визуализацией волновой функции.

**Задачи**:
- [ ] Создать компонент `/src/components/FFTChannelEditor.tsx`:
  - Табы для переключения между каналами R/G/B/A
  - Редактор DC-компоненты:
    - Слайдер `dcAmplitude` (0.0 - 2.0)
    - Слайдер `dcPhase` (0 - 2π) с круговым виджетом
  - Редактор коэффициентов (до 8 на канал):
    - Кнопка добавления/удаления коэффициентов
    - Слайдеры amplitude, phase для каждого
    - Числовые поля freqX, freqY, freqZ
- [ ] Добавить визуализацию спектра:
  - Миниатюрная диаграмма частотных компонентов
  - Предпросмотр влияния на внешний вид
- [ ] Создать пресеты коэффициентов:
  - "Crystal pulsation"
  - "Energy waves"
  - "Unstable core"
- [ ] Написать тесты
...
- UI остаётся отзывчивым при редактировании
...
**Описание**:
Добавить секцию для управления параметрами визуализации энергетических кубиков (EnergyCube props).

**Задачи**:
- [ ] Расширить FFT-редактор секцией "Visualization":
  - Dropdown `visualizationMode`: energy / amplitude / phase
  - Checkbox группа для `channelMask`: R, G, B, A
  - Слайдер `energyScale` (0.1 - 3.0)
  - Слайдер `glowIntensity` (0.0 - 2.0)
- [ ] Секция "Animation":
  - Checkbox `animate`
  - Слайдер `animationSpeed` (0.1 - 5.0)
  - Checkbox `rotate`
  - Слайдер `rotationSpeed` (0.0 - 2.0)
...
**Заголовок**: `Создание редактора для вертикальных стопок кубиков (CubeStackConfig)`

**Описание**:
Создать редактор для работы со стопками кубиков (ISSUE 20), позволяющий добавлять слои, настраивать переходы между ними и физику стопки.

**Задачи**:
- [ ] Создать компонент `/src/components/StackEditor.tsx`:
  - Визуальное представление слоёв (вертикальный список)
  - Кнопки добавления/удаления слоёв
  - Drag-and-drop для изменения порядка слоёв
- [ ] Редактор слоя (StackLayer):
  - Поле `name`
  - Слайдер `height` (0.5 - 3.0)
  - Ссылка на ParamEditor для `cubeConfig`
- [ ] Редактор переходов (StackTransition):
  - Dropdown `type`: blend / hard / gradient / noise
  - Слайдер `blendHeight` (0.0 - 1.0)
  - Dropdown `easing`: linear / smooth / ease-in / ease-out
  - Checkbox `blendNoise`, `blendGradients`
- [ ] Секция "Stack Physics":
  - Индикатор `isStable` (только чтение)
  - Отображение `totalWeight`
  - Слайдер `weightDistribution`
  - Индикатор `structuralIntegrity`
- [ ] Написать тесты

**Критерии приёмки**:
- Пользователь может создавать и редактировать стопки
- Слои можно перетаскивать для изменения порядка
- Переходы между слоями настраиваются
- Физика стопки отображается корректно
...
**Задачи**:
- [ ] Создать модуль `/src/lib/stack-presets.ts`:
  - Пресет "Stone Wall" (3 слоя камня)
  - Пресет "Wood Tower" (4 слоя дерева)
  - Пресет "Layered Earth" (почва → камень → трава)
  - Пресет "Magic Crystal Column" (FFT-кубики)
  - Пресет "Brick Chimney" (кирпич с градиентом)
- [ ] Создать компонент `/src/components/StackPresetPicker.tsx`:
  - Галерея миниатюр пресетов
  - Поиск по названию/тегам
  - Применение пресета к редактору
- [ ] Интегрировать с StackEditor:
  - Кнопка "Load Preset"
  - Возможность сохранения текущей стопки как пресета
- [ ] Написать тесты
...
**Описание**:
Добавить секцию для настройки LOD-параметров в ParamEditor, позволяющую пользователям контролировать уровень детализации на разных расстояниях.

**Задачи**:
- [ ] Создать секцию "LOD Settings" в ParamEditor:
  - Checkbox `enabled`
  - Dropdown для быстрого выбора профиля: Performance / Balanced / Quality
- [ ] Создать компонент `/src/components/LODConfigEditor.tsx`:
  - Редактор порогов расстояний для каждого уровня (0-4)
  - Слайдеры для `noiseOctaves`, `maxGradients`, `fftCoefficients`
  - Checkbox `enableNoise`, `enableBoundaryStitching`
  - Слайдер `transitionDuration`
- [ ] Добавить предпросмотр LOD-эффекта:
  - Показать как куб выглядит на каждом уровне LOD
  - Отобразить статистику производительности
- [ ] Интегрировать с useLODStatistics hook
- [ ] Написать тесты
...
### ISSUE 32: Интеграция расширенного AI в редактор (AI Generation Integration)

**Заголовок**: `Расширение AI-генератора для поддержки композитных описаний и контекста`

**Описание**:
Интегрировать расширенные возможности AI-генерации (ISSUE 19) в редактор для создания связанных кубиков и групп.

**Задачи**:
- [ ] Расширить `/src/components/PromptGenerator.tsx`:
  - Переключатель режима: Simple / Composite / Batch
  - Режим Composite:
    - Основное описание кубика
    - Добавление описаний соседей (+X, -X, +Y, -Y, +Z, -Z)
    - Выбор типа отношения: similar / contrast / gradient / complement
  - Режим Batch:
    - Список промптов (textarea с разделением по строкам)
    - Dropdown `grouping`: individual / related / themed
    - Поле для темы
- [ ] Добавить контекстную генерацию:
  - Кнопка "Generate based on selection"
  - Анализ выбранных кубиков для извлечения стиля
  - Отображение ExtractedStyle
- [ ] Интегрировать с CubeGrid/CubeStack:
  - Генерация кубика с учётом соседей
  - Генерация слоёв стопки с плавными переходами
- [ ] Написать тесты
...
### ISSUE 33: Режим коллаборативного редактирования (Collaboration Mode)

**Заголовок**: `Добавление индикаторов коллаборации в редактор параметров`

**Описание**:
Интегрировать ParamEditor с системой коллаборации (ISSUE 21-23) для отображения активности других участников.

**Задачи**:
- [ ] Расширить ParamEditor для режима коллаборации:
  - Индикатор "Currently editing" для каждого параметра
  - Отображение аватара участника, редактирующего секцию
  - Подсветка изменённых параметров цветом участника
- [ ] Добавить компонент `/src/components/CollaborativeParamEditor.tsx`:
  - Обёртка над ParamEditor с коллаборативными функциями
  - Интеграция с collaboration.ts модулем
  - Блокировка параметров при редактировании другим участником
- [ ] Добавить историю изменений:
  - Показывать последние изменения других участников
  - Возможность откатить изменения (для owner/editor)
- [ ] Написать тесты
...
### ISSUE 34: Унифицированный редактор (Unified Editor)

**Заголовок**: `Объединение всех редакторов в единый адаптивный интерфейс`

**Описание**:
Объединить все редакторы (ParamEditor, FFTParamEditor, StackEditor) в единый компонент с адаптивным интерфейсом.

**Задачи**:
- [ ] Создать компонент `/src/components/UnifiedEditor.tsx`:
  - Автоматическое определение типа объекта (SpectralCube / FFTCubeConfig / CubeStackConfig)
  - Переключатель режимов в заголовке
  - Единый API для всех типов
- [ ] Добавить вкладки для организации секций:
  - "Appearance" — base, gradients, noise
  - "Physics" — material, density, energy (для FFT)
  - "Advanced" — boundary, LOD, FFT channels
  - "AI" — prompt generator, presets
- [ ] Оптимизировать для мобильных устройств:
  - Свёрнутые секции по умолчанию
...
## Оценка объёма работ Фазы 5

| ISSUE | Сложность | Приоритет |
|-------|-----------|-----------|
| 25. Редактор границ | Низкая | Критический |
| 26. Базовый FFT-редактор | Средняя | Критический |
| 27. FFT-каналы | Высокая | Высокий |
| 28. Визуализация энергии | Средняя | Высокий |
| 29. Редактор стопок | Высокая | Критический |
| 30. Шаблоны стопок | Низкая | Средний |
| 31. LOD-настройки | Средняя | Средний |
| 32. AI-интеграция | Высокая | Высокий |
| 33. Коллаборация | Средняя | Средний |
| 34. Унифицированный редактор | Высокая | Высокий |

---

## Следующие шаги

После завершения Фазы 5 планируется:

- **Фаза 6**: Публикация и шаринг кубиков (галерея, share-ссылки, социальные функции)
